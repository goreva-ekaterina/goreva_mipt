...
def predict_number(number: int = 1) -> int:
    """функция делит диапазон, в котором загадано число, пополам. Делает
    предположение. Дальше сверяет это предположение с загаданным числом.
    И если не угадывает, то корректирует диапазон и делит новый также пополам.
